...
\begin{rem}
  \begin{itemize}
    ...
    \item (c) suggests that the "worst-approximable" number is $\theta = [1,1,1,\dots]$. 
...
More specifically, using tools we develp in Chapter 3, one can easily show that
...
Serge Lang in 1967 proved that if $\alpha$ is reals quadratic then
\[ N_\alpha(\frac{1}{x^2},Q) = c_\alpha \cdot \log(Q) + \mathcal{O}(1)\text{. } (c_\alpha > 0). \]
He mentioned that it would seem quite difficult to prove an asymptotic result por algebraic $\alpha$, let alone transcendend.\\
Adams showed
